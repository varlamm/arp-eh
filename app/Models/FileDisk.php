...
class FileDisk extends Model
{
    ...
    public static function validateCredentials($credentials, $disk)
    {
        ...
        try {
            ...
            \Storage::disk($prefix.$disk)->put($root.'xcelerate_temp.text', 'Check Credentials');

            if (\Storage::disk($prefix.$disk)->exists($root.'xcelerate_temp.text')) {
                $exists = true;
                \Storage::disk($prefix.$disk)->delete($root.'xcelerate_temp.text');
            }
        } catch (\Exception $e) {
            $exists = false;
        }
